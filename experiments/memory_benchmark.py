"""Core result 2 -- the real low-bit KV cache reduces both the stored cache and the peak GPU memory.

Stores the KV cache as actually packed low-bit integers (int8/int4/int3/int2) plus KIVI scales
and dequantizes only one layer at a time at attention time. Prints, per (precision x batch):
the real stored cache size (MB) and the measured peak GPU memory (GB).

    uv run experiments/memory_benchmark.py

Expected (report Tables 11-12): int4 cache ~3.6x smaller than fp16; at batch 128 peak ~46.8 -> 27.2 GB;
...
print("%-6s %6s  %14s  %8s  %6s" % ("prec", "batch", "cacheMB(real)", "peakGB", "sec"))
print("-" * 50)
for bits in bitlist:
